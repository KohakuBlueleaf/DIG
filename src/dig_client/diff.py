...
def generate(
    ...
    num_gen = prompt_embeds.size(0)
    if isinstance(seeds, int):
        if seeds == -1:
            seeds = [randint(0, 2**32 - 1) for _ in range(num_gen)]
        else:
            seeds = [seeds for _ in range(num_gen)]
    elif isinstance(seeds, list):
        if len(seeds) != num_gen:
            raise ValueError(
                f"Expected number of seeds to be {num_gen}, but got {len(seeds)}"
            )
        seeds = [seed if seed != -1 else randint(0, 2**32 - 1) for seed in seeds]
    else:
        raise ValueError(f"Invalid type for seeds: {type(seeds)}")
